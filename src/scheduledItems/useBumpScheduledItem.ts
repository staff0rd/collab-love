...
import { bumpScheduledItem } from "./bumpScheduledItem.ts";
...
type BumpVariables = { item: ScheduledItem; target: SnoozeTarget };

export const useBumpScheduledItem = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ item, target }: BumpVariables) => bumpScheduledItem(item, target, new Date()),
